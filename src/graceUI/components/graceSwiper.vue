<template name="graceSwiper">
	<view>
		<swiper class="grace-swiper" autoplay="true" :indicator-dots="indicatorDots" indicator-color="rgba(255, 255, 255, 1)" indicator-active-color="#fff" :style="{height : swiperHeight + 'px'}" :interval="interval">
			<swiper-item v-for="(item, index) in items" :key="index">
				<!-- <navigator :url='item.path' :open-type="item.openType"> -->
					<!-- <image :src='item.pic' @load='imgLoad' :id="swiperId + '-item-1'" mode='widthFix'></image> -->
					<image :src='item.pic' @load='imgLoad' :id="swiperId + '-item-1'" style="height:330upx;widht:100%;"></image>
